...
if VER >= 3:
    from BaseHTTPRequestHandlerv3 import BaseHTTPRequestHandler
      ...
      if self.handshaked is False:
         ...
         if not data:
            ...
            if b'\r\n\r\n' in self.headerbuffer:
               ...
               # Modification : HTTPRequest fait appel a bz2 dont la librairie n'est pas dans Libre Office.
               # Il faut donc passer par un autre moyne pour avoir headers['Sec-WebSocket-Key']
               # On utilise pour cela le code d'une autre web socket.
...
def Serveforever(server):
    while True:
        writers = []
        for fileno in server.listeners:
            if fileno == server.serversocket:
               continue
            client = server.connections[fileno]
            if client.sendq:
               writers.append(fileno)

        if server.selectInterval:
            rList, wList, xList = select(server.listeners, writers, server.listeners, server.selectInterval)
        else:
            rList, wList, xList = select(server.listeners, writers, server.listeners)

        for ready in wList:
            client = server.connections[ready]
            try:
               while client.sendq:
               ...
               client.client.close()
               client.handleClose()
               del server.connections[ready]
               server.listeners.remove(ready)

        for ready in rList:
            if ready == server.serversocket:
               try:
                  sock, address = server.serversocket.accept()
                  newsock = server._decorateSocket(sock)
                  newsock.setblocking(0)
                  fileno = newsock.fileno()
                  server.connections[fileno] = server._constructWebSocket(newsock, address)
                  server.listeners.append(fileno)
               except Exception as n:
                  if sock is not None:
                     sock.close()
            else:
               if ready not in server.connections:
                   continue
               client = server.connections[ready]
               try:
                  client._handleData()
        ...
        for failed in xList:
            if failed == server.serversocket:
               server.close()
               raise Exception('server socket failed')
            else:
               if failed not in server.connections:
                  continue
               client = server.connections[failed]
               client.client.close()
               client.handleClose()
               del server.connections[failed]
               server.listeners.remove(failed)
